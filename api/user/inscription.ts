...
export const getInscriptions = async ({
  ...
  const { data } = await axios.get<getInscriptionResponse>(`${API_URL}/inscriptions?page=${page}&limit=${limit}`)
  return data
}

export const getInscription = async (id: string): Promise<Inscription> => {
  const { data } = await axios.get<Inscription>(`${API_URL}/inscription/${id}`)
  return data
}
...
export const createInscription = async ({
  ...
  const { data } = await axios.post<Inscription>(`${API_URL}/inscription`, {
    dateOfBirth,
    guardian,
    lastName,
    name,
